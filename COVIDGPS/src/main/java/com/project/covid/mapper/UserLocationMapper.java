...
public interface UserLocationMapper {
	...
	@Select("SELECT * from UserLocation WHERE (mcode LIKE CONCAT('%/',#{mcode},'/%')or mcode LIKE CONCAT(#{mcode},'/','%') or mcode LIKE CONCAT('%/',#{mcode},''))and (( inTime <= #{inTime} and #{inTime} <= outTime) or (inTime<=#{outTime} and outTime<=#{outTime}))")
	List<LocationVO> getUidContactorLocationVO(@Param("mcode") String mcode,@Param("inTime")Date inTime,@Param("outTime")Date outTime);
